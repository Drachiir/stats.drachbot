...
def wave_distribution(patch, elo):
    ...
    return render_template("wave-distribution.html", wave_data=wave_data, patch=patch, elo= elo, games= games,
                           avg_elo=avg_elo, patch_list=patches, elos=elos, get_rank_url=util.get_rank_url)
...
# @app.route('/load_recap/<playername>/')
# def load_recap(playername):
#     session['visited_recap'] = True
#     return render_template('loading.html', playername=playername, url=f"/recap/{playername}")
#
# @app.route('/recap/', defaults={"playername": None})
# @app.route('/recap/<playername>/')
# def recap(playername):
#     if not playername:
#         return render_template("recap_home.html")
#     if not session.get('visited_recap'):
#         return redirect(f"/load_recap/{playername}")
#     session.pop('visited_recap', None)
#     referrer = request.referrer
#     if not referrer or '/load_recap' not in referrer:
#         return redirect(f"/load_recap/{playername}/")
#     if len(playername) > 13 and re.fullmatch(r'[0-9A-F]+', playername):
#         playerid = playername
#         api_profile = legion_api.getprofile(playerid, by_id=True)
...
#     if not api_stats["rankedLossesThisSeason"]:
#         api_stats["rankedLossesThisSeason"] = 0
#     path = f"Files/player_cache/{api_profile["playerName"]}_recap_11.json"
#     history = None
#     if os.path.isfile(path):
#         mod_date = datetime.fromtimestamp(os.path.getmtime(path), tz=timezone.utc)
#         date_diff = datetime.now(tz=timezone.utc) - mod_date
#         minutes_diff = date_diff.total_seconds() / 60
#         if minutes_diff > 6000:
#             os.remove(path)
#         else:
#             with open(path, "r") as f:
#                 history = json.load(f)
...
#             ["player_id", "player_name", "player_elo", "player_slot", "game_result", "elo_change", "legion",
#              "mercs_sent_per_wave", "kingups_sent_per_wave", "opener", "megamind", "spell", "workers_per_wave"]]
#         history = drachbot_db.get_matchistory(playerid, 0, 0, '11', earlier_than_wave10=True, req_columns=req_columns,
#                                               stats=api_stats, profile=api_profile, pname=playername)
#         with open(path, "w") as f:
#             json.dump(history, f, default=str)
#     winlose = [0, 0]
...
#     return render_template(
#         "profile.html",
#         api_profile=api_profile, api_stats=api_stats, get_rank_url=util.get_rank_url, winrate=util.custom_winrate, playername=playername, short_history = short_history,
#         winlose=winlose, elochange=util.plus_prefix(elochange), playerurl = f"/profile/{playername}/", values=values,
#         labels=labels, games=games, wave1 = wave1_percents, mms = mms, openers = openers, get_cdn = util.get_cdn_image,
#         spells = spells, player_dict=player_dict, profile=True, plus_prefix=util.plus_prefix, patch_list = patches2,
#         player_rank=player_rank, playerid = playerid)
